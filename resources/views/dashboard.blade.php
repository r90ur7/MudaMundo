                    ...
                    <form action="{{ route('dashboard') }}"
                        ...
                        <div class="mt-4 flex justify-between items-center">
                            ...
                            <p class="text-sm text-gray-400 dark:text-gray-500">
                                {{ isset($mudas) ? $mudas->total() : 0 }}
                                {{ Str::plural('muda', isset($mudas) ? $mudas->total() : 0) }}
                                encontrada{{ isset($mudas) && $mudas->total() != 1 ? 's' : '' }}
                            </p>

                            <!-- Lado direito - Botões -->
                            <div class="flex gap-3">
                                <!-- Exemplo de "Busca: ..." removível, se quiser exibir no topo do form -->
                                @if(request('search'))
                                    <span class="inline-flex items-center gap-1 px-3 py-1 rounded-full text-xs font-medium bg-emerald-100 text-emerald-700 dark:bg-emerald-700 dark:text-emerald-100">
                                        Busca: {{ is_array(request('search')) ? e(request('search')[0]) : e(request('search')) }}
                                        <a href="{{ route('dashboard', request()->except('search')) }}" class="ml-1 hover:text-emerald-900">
                                            <!-- ícone de fechar, etc. -->
                                    ...
                                    <div class="p-4 md:p-6">
                                        <div class="flex justify-between items-start">
                                            ...
                                            <span class="inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium
                                                {{ $muda->status?->nome === 'Disponível' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800' }}">
